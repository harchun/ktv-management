...
app.get('/api/customer-relations', authenticate, async (req, res) => {
  try {
    // Get cadre summary (all levels, last 60 days)
    const [cadres] = await pool.execute("SELECT cad.`幹部編號`, cad.`姓名`, cad.`暱稱`, COUNT(ds.`營業編號`) as 來訪次數, SUM(ds.`人數`) as 總人數 FROM daily_sales ds LEFT JOIN cadres cad ON ds.`幹部` = cad.`姓名` WHERE ds.`日期` >= DATE_SUB(CURDATE(), INTERVAL 60 DAY) GROUP BY cad.`幹部編號`, cad.`姓名`, cad.`暱稱` ORDER BY 來訪次數 DESC");
